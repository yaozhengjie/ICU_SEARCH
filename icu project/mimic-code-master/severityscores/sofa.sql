...
(
  ...
  WHERE valuenum IS NOT NULL--选择数据处理
  AND itemid IN
  (
  ...
  AND valuenum != 0 ---chartevent，如果为0，那么就是数据太多，时间设置为1天，上下就有两天的数据了，这个地方有一定疑点
...
, echo2 as(
  ...
  left join echodata echo
    ...
    and echo.charttime > ie.intime - interval '7' day --前七天数据流，声波表上面的登记数据。why use the severe and one day
    and echo.charttime < ie.intime + interval '1' day --后一天数据流
  group by ie.icustay_id
)
, vaso_cv as -- icu的数据流，处理的
(
  select ie.icustay_id
    ...
    , max(case
            when itemid =  30044 then rate / coalesce(wt.weight,ec.weight) -- measured in mcgmin，非空数值
...
(
  ...
  and statusdescription != 'Rewritten' --重写的数据
  group by ie.icustay_id
)
, pafi1 as
(
